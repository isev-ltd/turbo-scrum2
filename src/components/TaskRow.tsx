...
export default function TaskRow({task, updateTaskName, editingTaskId, setEditingTaskId, updateTask}) {
    ...
    return (<div className="relative rounded bg-slate-100 shadow p-4 text-slate-800 flex gap-2 items-center">
        ...
        </div> : <button type="button" className="flex-grow text-left" onClick={() => setEditingTaskId(task.id)}>{task.text}</button>}
        {time(task)}
        <DropdownMenu>
            <Menu.Item>
                {({ active }) => (
                    <button
                        onClick={() => {updateTask('is_completed', !task.is_completed)}}
            ...
            <Menu.Item>
                {({ active }) => (
                    <button
                        onClick={() => {updateTask('is_blocked', !task.is_blocked)}}
                        className={classNames(
                            active ? 'bg-gray-100 text-gray-900' : 'text-gray-700',
                            'block px-4 py-2 text-sm w-full text-left'
                        )}
...
function taskNameInput(task, text, setText, cancelUpdateTaskName) {
    ...
    return (
        <div className="relative flex-grow">
            <input autoFocus onFocus={handleFocus} type="text" name="account-number" id={`task-name-edit-${task.id}`}
                   value={text}
